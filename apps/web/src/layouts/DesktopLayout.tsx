import { MessageCircle, Clock, Phone as PhoneIcon, Settings as SettingsIcon, type LucideIcon } from 'lucide-react';
...
export default function DesktopLayout({
  ...
  if (activeTab === 'chats') {
    if (showNewChat) {
      center = <EmptyState icon={<MessageCircle size={64} strokeWidth={1.5} />} title="Pick a contact to start chatting" />;
    } else if (selectedChat) {
      center = screens.conversation(selectedChat, false);
    } else {
      center = <EmptyState icon={<MessageCircle size={64} strokeWidth={1.5} />} title="Select a chat to start messaging" />;
    }
  } else if (activeTab === 'settings') {
    if (settingsSection === 'privacy') {
      ...
      center = <EmptyState icon={<SettingsIcon size={64} strokeWidth={1.5} />} title="Select a setting" />;
    }
  } else {
    center = (
      <EmptyState
